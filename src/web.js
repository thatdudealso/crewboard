...
function editableTicketChanges(body) {
  const changes = {};
  for (const key of ['title', 'body', 'status', 'assignee', 'priority', 'type', 'parent']) {
...
<style>
:root { color-scheme: dark; --bg:#0c0e11; --surface:#14181e; --surface-2:#1a1f27; --surface-3:#222830; --line:#2b323c; --text:#e8ecf0; --muted:#9aa3ad; --quiet:#6d7682; --accent:#c5ced8; --shadow-card:0 1px 2px rgba(0,0,0,.06),0 8px 20px rgba(0,0,0,.06); --shadow-panel:0 1px 2px rgba(0,0,0,.06),0 12px 28px rgba(0,0,0,.06),0 28px 56px rgba(0,0,0,.06); --radius:12px; --column-width:280px; }
* { box-sizing:border-box; } body { margin:0; min-height:100vh; background:var(--bg); color:var(--text); font:14px/1.45 "IBM Plex Sans","Segoe UI","Helvetica Neue",sans-serif; }
button,input,select,textarea { font:inherit; } button { cursor:pointer; } .app { min-height:100vh; display:grid; grid-template-columns:240px minmax(0,1fr); }
.sidebar { border-right:1px solid var(--line); padding:22px 12px; background:#101319; position:sticky; top:0; height:100vh; overflow:auto; } .brand { letter-spacing:.1em; font-weight:700; font-size:12px; text-transform:uppercase; display:flex; justify-content:space-between; align-items:center; padding:0 8px 20px; }
.live { color:var(--muted); font-size:10px; letter-spacing:.06em; } .nav-label { color:var(--quiet); letter-spacing:.08em; font-size:10px; font-weight:700; text-transform:uppercase; margin:16px 8px 6px; }
.nav-button,.project-button { border:0; color:var(--muted); background:transparent; width:100%; text-align:left; padding:8px 10px; border-radius:8px; display:flex; align-items:center; gap:8px; } .nav-button:hover,.project-button:hover,.nav-button.active,.project-button.active { color:var(--text); background:var(--surface-2); }
.origin { border:1px solid var(--line); color:var(--muted); border-radius:999px; padding:1px 6px; font-size:10px; text-transform:uppercase; letter-spacing:.04em; } .main { min-width:0; padding:24px; }
.topbar { display:flex; justify-content:space-between; align-items:flex-start; gap:16px; margin-bottom:20px; } h1 { font-size:26px; line-height:1.15; margin:0 0 4px; letter-spacing:-.03em; font-weight:650; } h2 { font-size:16px; margin:0; letter-spacing:-.01em; font-weight:650; } p { color:var(--muted); margin:4px 0; } .actions { display:flex; gap:8px; flex-wrap:wrap; }
.button { border:1px solid var(--line); color:var(--text); background:var(--surface); border-radius:8px; padding:8px 12px; box-shadow:var(--shadow-card); } .button:hover { background:var(--surface-2); } .button.primary { background:var(--accent); color:#101319; border-color:var(--accent); font-weight:650; } .button.danger { color:#e5c2c2; }
.board { display:grid; grid-auto-flow:column; grid-auto-columns:var(--column-width); gap:14px; overflow-x:auto; padding-bottom:16px; align-items:stretch; } .column { width:var(--column-width); min-width:var(--column-width); max-width:var(--column-width); min-height:480px; border:1px solid var(--line); background:var(--surface); border-radius:var(--radius); padding:12px; box-shadow:var(--shadow-panel); display:flex; flex-direction:column; }
.column-heading { display:flex; align-items:center; justify-content:space-between; color:var(--muted); text-transform:uppercase; font-size:11px; letter-spacing:.08em; font-weight:700; padding:2px 2px 12px; flex:0 0 auto; } .count { background:var(--surface-3); border-radius:999px; min-width:22px; text-align:center; padding:1px 6px; color:var(--text); }
.ticket-list { flex:1 1 auto; min-height:360px; display:flex; flex-direction:column; gap:10px; } .ticket { border:1px solid var(--line); background:var(--surface-2); border-radius:10px; padding:11px; box-shadow:var(--shadow-card); } .ticket:hover { border-color:#3d4652; } .ticket.dragging { opacity:.45; } .ticket.story { border-left:3px solid #8b95a1; } .ticket.task { border-left:3px solid #5f6b78; } .ticket.subtask { border-left:3px solid #44505c; margin-left:10px; }
.ticket-meta,.small { color:var(--muted); font-size:11px; } .ticket-title { margin:6px 0 8px; font-weight:650; line-height:1.3; } .ticket-footer { display:flex; justify-content:space-between; gap:8px; align-items:center; flex-wrap:wrap; } .chip { color:var(--muted); border:1px solid var(--line); border-radius:999px; padding:2px 7px; font-size:10px; } .chip.leader { color:var(--text); border-color:#4a5562; background:var(--surface-3); } .progress { color:var(--quiet); font-size:11px; }
.panel { border:1px solid var(--line); border-radius:var(--radius); padding:16px; background:var(--surface); box-shadow:var(--shadow-panel); margin-bottom:16px; } .panel-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:14px; } .project-card { border:1px solid var(--line); border-radius:10px; padding:14px; background:var(--surface-2); }
...
.message { border-left:2px solid var(--line); padding:10px 12px; background:var(--surface-2); border-radius:0 8px 8px 0; margin:8px 0; } .message p { color:var(--text); white-space:pre-wrap; } .empty { color:var(--muted); border:1px dashed var(--line); border-radius:10px; padding:28px; text-align:center; }
.modal-backdrop { position:fixed; inset:0; background:rgba(0,0,0,.58); display:none; align-items:center; justify-content:center; padding:18px; } .modal-backdrop.open { display:flex; } .modal { width:min(760px,100%); max-height:92vh; overflow:auto; border:1px solid var(--line); background:var(--surface); border-radius:14px; padding:20px; box-shadow:var(--shadow-panel); }
.modal-head { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:12px; } .two-col { display:grid; grid-template-columns:1fr 1fr; gap:12px; } .notice { color:var(--muted); font-size:12px; margin:8px 0; } .hidden { display:none; } .breadcrumbs { color:var(--muted); font-size:12px; margin-bottom:10px; } .breadcrumbs span + span::before { content:" / "; color:var(--quiet); } .agents-row { display:flex; flex-wrap:wrap; gap:8px; margin-top:10px; } .attention-item { border:1px solid var(--line); border-radius:10px; padding:14px; background:var(--surface-2); margin:0 0 10px; } .attention-item a { color:var(--text); text-decoration:none; } .attention-item a:hover { text-decoration:underline; } .attention-meta { display:flex; flex-wrap:wrap; gap:6px; margin:8px 0 0; } .toggle-row { display:flex; align-items:center; gap:10px; color:var(--muted); font-size:12px; }
...
<body>
<div class="app"><aside class="sidebar"><div class="brand">Crewboard <span class="live">LIVE</span></div><div class="nav-label">Views</div><button class="nav-button active" data-view="board">Board</button><button class="nav-button" data-view="messages">Message board</button><button class="nav-button" data-view="agents">Agents</button><button class="nav-button" data-view="attention">GitHub attention</button><button class="nav-button" data-view="projects">Projects</button><div class="nav-label">Active projects</div><div id="project-nav"></div><div class="nav-label">Pending approval</div><div id="pending-nav"></div></aside><main class="main"><div id="app"></div></main></div>
...
async function refreshAttention({force=false}={}){if(state.attentionLoading)return;state.attentionLoading=true;state.attentionError='';if(force||state.view==='attention')render();try{state.attention=await api('/api/github/attention?all='+(state.attentionAll?'1':'0'));state.attentionFetchedAt=state.attention.refreshedAt||new Date().toISOString();}catch(error){state.attentionError=error.message;state.attention={available:false,sourceFound:false,error:error.message,items:[],repos:[]};state.attentionFetchedAt=null;}finally{state.attentionLoading=false;if(state.view==='attention')render();}}
...
function assigneeChip(item,ticket){if(!ticket.assignee)return '<span class="chip">unassigned</span>';const leader=agentRole(item,ticket.assignee)?.role==='leader';return '<span class="chip '+(leader?'leader':'')+'">@'+escapeHtml(ticket.assignee)+(leader?' · leader':'')+'</span>';}
function progressText(ticket){if(!ticket.progress||!ticket.progress.total||ticket.type==='subtask')return '';return '<span class="progress">'+ticket.progress.completed+'/'+ticket.progress.total+'</span>';}
function parentLabel(item,ticket){if(!ticket.parent)return '';const parent=item.tickets.find((candidate)=>candidate.id===ticket.parent);return parent?'<div class="small">under '+escapeHtml(parent.type)+' · '+escapeHtml(parent.id)+'</div>':'';}
function ticketCard(item,ticket){return '<article class="ticket '+escapeHtml(ticket.type||'task')+'" draggable="true" data-ticket="'+escapeHtml(ticket.id)+'" data-project="'+escapeHtml(item.id)+'"><div class="ticket-meta">'+escapeHtml(ticket.id)+' · '+escapeHtml(ticket.type||'task')+' · '+escapeHtml(ticket.priority)+'</div>'+parentLabel(item,ticket)+'<div class="ticket-title">'+escapeHtml(ticket.title)+'</div><div class="ticket-footer">'+assigneeChip(item,ticket)+progressText(ticket)+'<span class="small">'+ticket.messages.length+' msg</span></div></article>';}
function orderedColumnTickets(item,status){const tickets=item.tickets.filter((ticket)=>ticket.status===status);const byParent=new Map();for(const ticket of tickets){const key=ticket.parent||'';if(!byParent.has(key))byParent.set(key,[]);byParent.get(key).push(ticket);}const rank={story:0,task:1,subtask:2};const roots=tickets.filter((ticket)=>!ticket.parent||!tickets.some((candidate)=>candidate.id===ticket.parent)).sort((a,b)=>(rank[a.type]??9)-(rank[b.type]??9)||(a.position||0)-(b.position||0));const ordered=[];const visit=(ticket)=>{ordered.push(ticket);const children=(byParent.get(ticket.id)||[]).sort((a,b)=>(a.position||0)-(b.position||0));for(const child of children)visit(child);};for(const root of roots)visit(root);for(const ticket of tickets)if(!ordered.includes(ticket))ordered.push(ticket);return ordered;}
function boardForProject(item){if(!item.available)return '<section class="empty">'+escapeHtml(item.error||'This project board is unavailable.')+'</section>';const leaders=(item.agents||[]).filter((agent)=>agent.role==='leader');return '<section class="panel"><div class="topbar"><div><h2>'+escapeHtml(item.name)+'</h2><p>'+escapeHtml(item.organization)+' · <span class="origin">'+escapeHtml(item.origin)+'</span></p>'+(leaders.length?'<div class="agents-row">'+leaders.map((agent)=>'<span class="chip leader">@'+escapeHtml(agent.name)+' · leader</span>').join('')+'</div>':'')+'</div></div><div class="board">'+item.columns.map((status)=>{const tickets=orderedColumnTickets(item,status);return '<section class="column" data-column="'+escapeHtml(status)+'" data-project="'+escapeHtml(item.id)+'"><div class="column-heading"><span>'+escapeHtml(status)+'</span><span class="count">'+tickets.length+'</span></div><div class="ticket-list">'+tickets.map((ticket)=>ticketCard(item,ticket)).join('')+'</div></section>';}).join('')+'</div></section>';}
...
function renderAgents(){const selected=project();const agents=selected?.agents||[];return '<section class="panel"><h2>Fleet agents</h2><p>Roles live with the board. The leader triages and assigns; scouts and workers are ordinary agents.</p><div class="panel-grid">'+(agents.map((agent)=>'<article class="project-card"><div class="small">'+(agent.role==='leader'?'<span class="chip leader">leader</span>':'<span class="chip">'+escapeHtml(agent.role)+'</span>')+'</div><h2>@'+escapeHtml(agent.name)+'</h2><p>'+escapeHtml(agent.description)+'</p></article>').join('')||'<p class="empty">No agents registered.</p>')+'</div></section>';}
...
function render(){renderNavigation();const selected=project();const titles={board:'Board',messages:'Message board',agents:'Agents',attention:'GitHub attention',projects:'Projects'};const subtitle=state.view==='board'?(selected?selected.name+' · live ticket state':'Fleet overview'):state.view==='messages'?'Threaded coordination, kept with the work':state.view==='agents'?'Fleet roles and the board leader':state.view==='attention'?'What on GitHub needs the captain':'Approval and organization';let content=state.view==='board'?renderBoard():state.view==='messages'?renderMessages():state.view==='agents'?renderAgents():state.view==='attention'?renderAttention():renderProjects();document.querySelector('#app').innerHTML='<div class="topbar"><div><h1>'+escapeHtml(titles[state.view]||'Board')+'</h1><p>'+escapeHtml(subtitle)+'</p></div><div class="actions">'+(selected&&state.view!=='projects'&&state.view!=='attention'?'<button class="button" data-open-new-ticket="true">New ticket</button>':'')+(state.view==='attention'?'<button class="button" data-refresh-attention="true">Refresh</button>':'<button class="button" data-refresh="true">Refresh</button>')+'</div></div>'+content;bindBoard();}
